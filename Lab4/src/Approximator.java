...
public class Approximator {
    ...
    public Result cubic(double[][] functionTable) {
        ...
        return new Result(ApproximationType.CUBIC, solution, function, deviation);
    }

    public Result square(double[][] functionTable) {
        ...
        double deviation = deviationMeasure(functionTable, function);
        return new Result(ApproximationType.QUADRATIC, solution, function, deviation);
    }

    public Result linear(double[][] functionTable) {
        ...
        double deviation = deviationMeasure(functionTable, function);
        return new Result(ApproximationType.LINEAR, solution, function, deviation, linearCorrelation(functionTable));
    }

    public Result exponential(double[][] functionTable) {
        ...
        return new Result(ApproximationType.EXPONENTIAL, coefficients, f, deviationMeasure(functionTable, f));
    }

    public Result logarithmic(double[][] functionTable) {
        double[][] modifiedFunctionTable = Arrays.stream(functionTable).map(double[]::clone).toArray(double[][]::new);
        for (double[] xy: modifiedFunctionTable) {
            xy[0] = Math.log(xy[0]);
        }
        ...
        return new Result(ApproximationType.LOGARITHMIC, coefficients, f, deviationMeasure(functionTable, f));
    }

    public Result power(double[][] functionTable) {
        double[][] modifiedFunctionTable = Arrays.stream(functionTable).map(double[]::clone).toArray(double[][]::new);
        for (double[] xy: modifiedFunctionTable) {
            xy[0] = Math.log(xy[0]);
            xy[1] = Math.log(xy[1]);
        ...
        return new Result(ApproximationType.POWER, coefficients, f, deviationMeasure(functionTable, f));
    }
